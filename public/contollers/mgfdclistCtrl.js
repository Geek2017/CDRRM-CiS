angular.module('newApp').controller('mgfdclistCtrl', function($firebaseArray, $scope, $http, $timeout) {
    ...
    firebase.database().ref('/cswdo/mgfdc/').orderByChild('uid').on("value", function(snapshot) {
        ...
        $timeout(function() {
            ...
            $("#pfilter").change(function() {
                ...
                window.location.href = "#mgfdclist"
            });
            ...
            for (i = 0; i < numPages; i++) {
                var pageNum = i + 1;
                $('#nav').append('<a href="#mgfdclist" rel="' + i + '">' + pageNum + '</a>');
            }
    ...
    $scope.selectUser = function(users) {
        ...
        document.getElementById("start").value = users.start;
        document.getElementById("end").value = users.end;
    ...
    $scope.updateUser = function() {
        var ref2 = firebase.database().ref("/cswdo/mgfdc/" + id.$id);
        ref2.update({
    ...
    $scope.deleteUser = function() {
        var ref = firebase.database().ref("/cswdo/mgfdc/" + id.key);
        ref.remove()
        .catch(function(error) {
            console.log("Login Failed!", error.message);
            $("#notif").append('<div class="alert alert-danger fade in"><button class="close" data-dismiss="alert">×</button><i class="fa-fw fa fa-check"></i><strong>Error</strong> ' + error.message + '</div>');
    ...
    $('#newmgfdc').on('submit', function(e) {
        ...
        var data = {
            ...
            noOfFamilies: $scope.clickedUser.noOfFamilies,
            noOfMeals: $scope.clickedUser.noOfMeals,
            start: $('#start').val(),
            end: $('#end').val(),
            meals: withoutLast2,
        }
        ...
        var updates = {};
        updates['/cswdo/mgfdc/'  + id.key] = data;
    ...
    $scope.addtr = function() {
        $("#appendhere").append(' <tr><td class="col-md-2"> <label class="input"> <input type="date" name="remarks" placeholder="" required></label></td></td><td class="col-md-2"><label class="input"> <input type="date" name="remarks" placeholder=""></label></td></td><td class="col-md-2"><label class="input"> <input type="date" name="amount" value="" class="" /></label></td></tr>');
        cnt++;
