angular.module('newApp').controller('donationlistCtrl', function($firebaseArray, $scope, $http, $timeout) {
    ...
    firebase.database().ref('/cswdo/donations/').orderByChild('uid').on("value", function(snapshot) {
        ...
        $timeout(function() {
            ...
            $("#pfilter").change(function() {
                ...
                window.location.href = "#donationlist"
            });
            ...
            for (i = 0; i < numPages; i++) {
                var pageNum = i + 1;
                $('#nav').append('<a href="#donationlist" rel="' + i + '">' + pageNum + '</a>');
            }
    ...
    $scope.updateUser = function() {
        var ref2 = firebase.database().ref("/cswdo/donations/" + id.$id);
        ref2.update({
    ...
    $scope.deleteUser = function() {
        var ref = firebase.database().ref("/cswdo/donations/" + id.key);
        ref.remove()
        .catch(function(error) {
            console.log("Login Failed!", error.message);
            $("#notif").append('<div class="alert alert-danger fade in"><button class="close" data-dismiss="alert">×</button><i class="fa-fw fa fa-check"></i><strong>Error</strong> ' + error.message + '</div>');
    ...
    $('#newcf').on('submit', function(e) {
        ...
        var name = $scope.clickedUser.name;
        var address = $scope.clickedUser.address;
        ...
        var data = {
            ...
            address: $scope.clickedUser.address,
            donations: withoutLast2,
        }

        var updates = {};
        // console.log(id.key);
        updates['/cswdo/donations/'  + id.key] = data;
