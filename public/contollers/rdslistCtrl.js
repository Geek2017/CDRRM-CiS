angular.module('newApp').controller('rdslistCtrl', function($firebaseArray, $scope, $http, $timeout) {
    ...
    firebase.database().ref('/cswdo/rds/').orderByChild('uid').on("value", function(snapshot) {
        ...
        $timeout(function() {
            ...
            for (i = 0; i < numPages; i++) {
                var pageNum = i + 1;
                $('#nav').append('<a href="#rdslist" rel="' + i + '">' + pageNum + '</a>');
            }
    ...
    $scope.selectUser = function(users) {
        ...
        $scope.clickedUser = users;
        id = users;
        document.getElementById("dateOccurance").value = users.dateOccurance;
        // document.getElementById("editDateOfEvacuation").value = users.dateOfEvacuation;
    ...
    $scope.updateUser = function() {
        var ref2 = firebase.database().ref("/cswdo/rds/" + id.$id);
        ref2.update({
    ...
    $scope.deleteUser = function() {
        var ref = firebase.database().ref("/cswdo/rds/" + id.key);
        ref.remove()
        .catch(function(error) {
            console.log("Login Failed!", error.message);
            $("#notif").append('<div class="alert alert-danger fade in"><button class="close" data-dismiss="alert">×</button><i class="fa-fw fa fa-check"></i><strong>Error</strong> ' + error.message + '</div>');
    ...
    $('#newrds').on('submit', function(e) {
        ...
        var data = {
            ...
            disaster: $scope.clickedUser.disaster,
            placeOccurance: $scope.clickedUser.placeOccurance,
            dateOccurance: $('#dateOccurance').val(),
            from: $scope.clickedUser.from,
            position: $scope.clickedUser.position,
            relief: withoutLast2,
        }

        var updates = {};
        // console.log(id.key);
        updates['/cswdo/rds/'  + id.key] = data;
